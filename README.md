...
- **格式转换**：请求/非流式响应/流式 SSE 双向转换，覆盖 system、图片（base64/URL）、工具调用（tool_calls ↔ tool_use/tool_result）、推理内容（thinking ↔ reasoning_content；Anthropic signature 不暴露给客户端）
...
- **MCP 服务器**：`/mcp`（Streamable HTTP，POST/GET/DELETE 同一端点），让用户在自己的 AI Agent 中以 `Bearer th-xxx` 接入网关查询自己的数据，6 个只读工具按 user 隔离
...
# 启动（首次启动自动创建管理员账号；默认 DB 写入 data/tokenhub.db）
PORT=8080 ADMIN_USERNAME=admin ADMIN_PASSWORD=admin123 ./tokenhub
...
# Anthropic 格式（同一个 Key；两种头都支持，下例用 Bearer 与 OpenAI 风格一致）
...
| `list_my_keys` | 列出自己的下游 Key（id/name/key_prefix/disabled/last_used_at/created_at，不含明文/哈希） |
| `list_my_logs` | 分页查询自己的请求日志（page/size 默认 1/20，max 100；支持 model/status/days/trace_id 过滤，days 默认 7、最大 90） |
| `get_trace_detail` | 查询单次调用的完整 trace（下游 + 所有上游尝试）；越权访问返回 `not_found=true` 不报错 |
| `get_my_stats` | 按天聚合用量 + 按模型 TOP（支持 days，默认 7、最大 90） |
| `get_my_account` | 返回当前用户账号信息（用户名、角色、累计消费、注册时间） |
...
|---|---|---|
| `PORT` | 8080 | 监听端口 |
| `DB_PATH` | data/tokenhub.db | SQLite 文件路径 |
...
internal/
  ...
  api/               管理/用户 REST API（handlers.go / admin.go，含 admin_test.go）
  mcp/               MCP 服务器：6 个工具（只读，按 user 隔离）+ Streamable HTTP transport（含 server_test.go）
...
```bash
# 首次运行前请先构建前端（go:embed all:dist 需要 internal/web/dist 存在）
cd web && npm install && npm run build && cd ..
...
- `internal/mcp`：`initialize` 协议握手、`tools/list` 数量与命名、4 个核心工具（list_models / list_my_keys / list_my_logs / get_trace_detail / get_my_account）的 happy path、跨用户隔离与越权防护（越权返回 `not_found=true`）

## 设计说明

- **重试边界**：上游响应首字节写往下游之前失败可静默重试（换 Key/换渠道）；流式已开始输出后失败，向前端发送错误帧并终止
- **Key 状态机**：`active` → 401/403 → `invalid`（不再参与轮询，可手动恢复）；429 → `rate_limited`（冷却 60s，结束后下一次成功调用自动回归 `active`）；连续失败仅计数，不摘除
